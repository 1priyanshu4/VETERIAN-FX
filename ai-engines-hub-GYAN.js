...
(() => {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // SELF-IMPROVING TRADE MEMORY & FEEDBACK SYSTEM
  // ═══════════════════════════════════════════════════════════════════════════
  // Stores all trade outcomes, learns from wins/losses, adapts model weights
  // ═══════════════════════════════════════════════════════════════════════════

  const TRADE_MEMORY_KEY = 'vetfx_trade_memory';
  const MODEL_WEIGHTS_KEY = 'vetfx_model_weights';
  const CANDLE_HISTORY_KEY = 'vetfx_candle_history';
  const BIAS_CACHE_KEY = 'vetfx_bias_cache';

  // Default equal weights for all 5 models (will adapt based on performance)
  ...
  // Minimum weight floor (prevent a model from being completely silenced)
  const MIN_MODEL_WEIGHT = 0.05;
  const MAX_MODEL_WEIGHT = 0.40;
  const PERFORMANCE_WINDOW = 30; // Last 30 trades for weight calculation

  // ── Trade Memory Store ──────────────────────────────────────────────────
  let tradeMemory = [];
  let modelWeights = { ...DEFAULT_MODEL_WEIGHTS };
  let candleHistory = {}; // { symbol: [ { o, h, l, c, t, v }, ... ] }
  let biasCache = {}; // { symbol: { bias: 'BULLISH'|'BEARISH'|'NEUTRAL', ema8, ema21, ema50, updatedAt } }

  function loadTradeMemory() {
    try {
      const saved = localStorage.getItem(TRADE_MEMORY_KEY);
      if (saved) tradeMemory = JSON.parse(saved);
      const weights = localStorage.getItem(MODEL_WEIGHTS_KEY);
      if (weights) modelWeights = JSON.parse(weights);
      const candles = localStorage.getItem(CANDLE_HISTORY_KEY);
      if (candles) candleHistory = JSON.parse(candles);
      const bias = localStorage.getItem(BIAS_CACHE_KEY);
      if (bias) biasCache = JSON.parse(bias);
    } catch (e) {
      tradeMemory = [];
  ...
  function saveTradeMemory() {
    try {
      localStorage.setItem(TRADE_MEMORY_KEY, JSON.stringify(tradeMemory));
      localStorage.setItem(MODEL_WEIGHTS_KEY, JSON.stringify(modelWeights));
      localStorage.setItem(CANDLE_HISTORY_KEY, JSON.stringify(candleHistory));
      localStorage.setItem(BIAS_CACHE_KEY, JSON.stringify(biasCache));
    } catch (e) {}
  }

  // Record a completed trade with full context
  function recordTradeOutcome(tradeRecord) {
    // tradeRecord: { id, symbol, modelKey, direction, entry, tp, sl, result('WIN'|'LOSS'|'BE'),
    //   exitPrice, pnlPct, bias, timestamp, duration, marketConditions, logic }
    tradeMemory.push(tradeRecord);

    // Cap memory at 500 trades to prevent localStorage overflow
    if (tradeMemory.length > 500) {
      tradeMemory = tradeMemory.slice(-500);
    }

    // Recalculate adaptive weights
    recalculateModelWeights();
    saveTradeMemory();

    console.log(`[TRADE MEMORY] Recorded ${tradeRecord.result} for ${tradeRecord.modelKey} on ${tradeRecord.symbol}. ` +
      `Model win rate: ${getModelWinRate(tradeRecord.modelKey).toFixed(1)}%`);
  }

  // Calculate win rate for a specific model over last N trades
  function getModelWinRate(modelKey) {
    const modelTrades = tradeMemory
      .filter(t => t.modelKey === modelKey)
      .slice(-PERFORMANCE_WINDOW);
    if (modelTrades.length < 3) return 50; // Not enough data, assume neutral
    const wins = modelTrades.filter(t => t.result === 'WIN').length;
    return (wins / modelTrades.length) * 100;
  }

  // Get model performance stats
  function getModelStats(modelKey) {
    const modelTrades = tradeMemory
      .filter(t => t.modelKey === modelKey)
      .slice(-PERFORMANCE_WINDOW);
    if (modelTrades.length === 0) return { winRate: 50, avgRR: 0, trades: 0, streak: 0, weight: modelWeights[modelKey] || 0.20 };

    const wins = modelTrades.filter(t => t.result === 'WIN');
    const losses = modelTrades.filter(t => t.result === 'LOSS');

    // Calculate current streak
    let streak = 0;
    for (let i = modelTrades.length - 1; i >= 0; i--) {
      if (i === modelTrades.length - 1) {
        streak = modelTrades[i].result === 'WIN' ? 1 : -1;
      } else if ((streak > 0 && modelTrades[i].result === 'WIN') || (streak < 0 && modelTrades[i].result === 'LOSS')) {
        streak += streak > 0 ? 1 : -1;
      } else {
        break;
      }
    ...
    const avgWinPnl = wins.length > 0 ? wins.reduce((s, t) => s + Math.abs(t.pnlPct || 0), 0) / wins.length : 0;
    const avgLossPnl = losses.length > 0 ? losses.reduce((s, t) => s + Math.abs(t.pnlPct || 0), 0) / losses.length : 0;

    return {
      winRate: (wins.length / modelTrades.length) * 100,
      avgRR: avgLossPnl > 0 ? (avgWinPnl / avgLossPnl) : 0,
      trades: modelTrades.length,
      streak: streak,
      weight: modelWeights[modelKey] || 0.20,
      lastTrades: modelTrades.slice(-5).map(t => t.result === 'WIN' ? 'W' : t.result === 'LOSS' ? 'L' : 'BE')
    };
  }

  // Recalculate model weights based on performance (self-improving)
  function recalculateModelWeights() {
    const modelKeys = ['aether9', 'evolvex', 'sentinel', 'unity', 'orbit'];
    const scores = {};
    let totalScore = 0;

    modelKeys.forEach(key => {
      const stats = getModelStats(key);
      // Score = winRate * avgRR (rewards both accuracy AND profitability)
      // Penalize models with negative streaks
      let score = (stats.winRate / 100) * Math.max(stats.avgRR, 0.5);
      if (stats.streak < -3) score *= 0.7; // Heavy penalty for 3+ loss streak
      if (stats.streak > 3) score *= 1.3;  // Boost for 3+ win streak
      score = Math.max(score, 0.1); // Floor to prevent zero weight
      scores[key] = score;
      totalScore += score;
    });
    ...
    if (totalScore > 0) {
      modelKeys.forEach(key => {
        let weight = scores[key] / totalScore;
        weight = Math.max(MIN_MODEL_WEIGHT, Math.min(MAX_MODEL_WEIGHT, weight));
        modelWeights[key] = weight;
      });

      // Normalize to sum to 1.0
      const sum = modelKeys.reduce((s, k) => s + modelWeights[k], 0);
      modelKeys.forEach(key => { modelWeights[key] /= sum; });
    }
  }

  // Analyze why a trade lost (mistake classification)
  function classifyMistake(tradeRecord) {
    const reasons = [];

    // Check if trade was against the bias
    if (tradeRecord.bias && tradeRecord.direction) {
      if ((tradeRecord.bias === 'BULLISH' && tradeRecord.direction === 'SELL') ||
          (tradeRecord.bias === 'BEARISH' && tradeRecord.direction === 'BUY')) {
        reasons.push('COUNTER_TREND');
      }
    }

    // Check if SL was too tight (PnL barely negative)
    if (tradeRecord.pnlPct && Math.abs(tradeRecord.pnlPct) < 0.3) {
      reasons.push('SL_TOO_TIGHT');
    }

    // Check if entry was bad (never went in favor at all)
    if (tradeRecord.maxFavorablePnl !== undefined && tradeRecord.maxFavorablePnl <= 0) {
      reasons.push('BAD_ENTRY');
    }

    // Check duration — very fast losses suggest wrong direction
    if (tradeRecord.duration && tradeRecord.duration < 300000) { // < 5 minutes
      reasons.push('INSTANT_REVERSAL');
    }

    return reasons.length > 0 ? reasons : ['MARKET_NOISE'];
  }

  // ── EMA Calculation from Candle History ──────────────────────────────────
  function calculateEMA(prices, period) {
    if (!prices || prices.length < period) return prices && prices.length > 0 ? prices[prices.length - 1] : 0;
    const k = 2 / (period + 1);
  ...
  // ── Market Structure Bias Determination (HTF) ───────────────────────────
  // Uses EMA 8/21/50 cross analysis + price position + swing structure
  function determineMarketBias(symbol) {
    const cached = biasCache[symbol];
    const now = Date.now();

    // Use cache if fresh (updated within last 5 minutes)
    if (cached && cached.updatedAt && (now - cached.updatedAt) < 300000) {
      return cached;
    }

    const candles = candleHistory[symbol];
    if (!candles || candles.length < 55) {
      // Not enough candle data — use live price trend as fallback
      return determineBiasFromLivePrice(symbol);
    }
    ...
    // Swing high/low detection (last 20 candles)
    const recent = candles.slice(-20);
    let higherHighs = 0, lowerLows = 0;
    for (let i = 2; i < recent.length; i++) {
      if (recent[i].h > recent[i - 1].h && recent[i - 1].h > recent[i - 2].h) higherHighs++;
      if (recent[i].l < recent[i - 1].l && recent[i - 1].l < recent[i - 2].l) lowerLows++;
    }
    ...
    if (ema8 > ema21 && ema21 > ema50) bullishScore += 3;
    if (ema8 < ema21 && ema21 < ema50) bearishScore += 3;

    // Price vs EMAs
    if (currentPrice > ema8 && currentPrice > ema21) bullishScore += 2;
    if (currentPrice < ema8 && currentPrice < ema21) bearishScore += 2;
    if (currentPrice > ema50) bullishScore += 1;
    if (currentPrice < ema50) bearishScore += 1;

    // Swing structure
    if (higherHighs > lowerLows + 1) bullishScore += 2;
    if (lowerLows > higherHighs + 1) bearishScore += 2;

    // EMA momentum (gap between fast and slow)
    const emaMomentum = (ema8 - ema50) / ema50;
    if (emaMomentum > 0.002) bullishScore += 1;
    if (emaMomentum < -0.002) bearishScore += 1;

    let bias = 'NEUTRAL';
    let confidence = 50;

    if (bullishScore >= 5 && bullishScore > bearishScore + 2) {
      bias = 'BULLISH';
      confidence = Math.min(95, 50 + bullishScore * 5);
    } else if (bearishScore >= 5 && bearishScore > bullishScore + 2) {
      bias = 'BEARISH';
    ...
    biasCache[symbol] = biasResult;
    saveTradeMemory();

    return biasResult;
  }

  // Fallback bias from live price when candle history is not available
  function determineBiasFromLivePrice(symbol) {
    const market = livePrices[symbol];
    if (!market || !market.price) {
      return { bias: 'NEUTRAL', confidence: 30, updatedAt: Date.now() };
    }

    const p = market.price;
    const changeStr = market.change || '+0.00%';
    const changePct = parseFloat(changeStr.replace('%', '')) || 0;
    const high = market.high || p;
    const low = market.low || p;
    const range = high - low;
    const pricePosition = range > 0 ? (p - low) / range : 0.5; // 0 = at low, 1 = at high

    // Check recent trade memory for this symbol to detect trend
    const recentTrades = tradeMemory
      .filter(t => t.symbol === symbol)
      .slice(-10);
    const recentWinningDir = recentTrades
      .filter(t => t.result === 'WIN')
      .map(t => t.direction);
    const buyWins = recentWinningDir.filter(d => d === 'BUY').length;
    const sellWins = recentWinningDir.filter(d => d === 'SELL').length;

    let bias = 'NEUTRAL';
    let confidence = 40;

    // Strong change signal
    if (changePct > 0.5 && pricePosition > 0.6) {
      bias = 'BULLISH';
      confidence = 55 + Math.min(20, changePct * 5);
    } else if (changePct < -0.5 && pricePosition < 0.4) {
      bias = 'BEARISH';
      confidence = 55 + Math.min(20, Math.abs(changePct) * 5);
    }

    // Historical winning direction reinforcement
    if (buyWins > sellWins + 2) {
      if (bias !== 'BEARISH') { bias = 'BULLISH'; confidence += 10; }
    } else if (sellWins > buyWins + 2) {
      if (bias !== 'BULLISH') { bias = 'BEARISH'; confidence += 10; }
    }

    return { bias, confidence, updatedAt: Date.now() };
  }

  // Fetch candle history for bias calculation (Binance for crypto, simulated EMA for forex)
  async function fetchCandleHistory(symbol) {
    const asset = ASSETS[symbol];
    if (!asset) return;

    if (asset.binance) {
      // Fetch 1H candles from Binance for EMA calculation
      try {
        const res = await fetch(`https://api.binance.com/api/v3/klines?symbol=${asset.binance}&interval=1h&limit=60`);
        if (res.ok) {
      ...
      // For forex/metals/commodities/indices — ensure robust 60-candle history for EMA bias
      if (!candleHistory[symbol] || candleHistory[symbol].length < 55) {
        const market = livePrices[symbol] || { price: asset.baseRate };
        const p = market.price;
        const high = market.high || p * 1.008;
        const low = market.low || p * 0.992;
        const range = Math.max(p * 0.005, high - low);
        const candles = [];
        const now = Date.now();
        const chgVal = parseFloat((market.change || '+0.25%').replace('%', '')) || 0;
        const driftPerHour = (chgVal / 100) / 24;

        for (let i = 59; i >= 0; i--) {
          const t = now - (i * 3600000);
          const center = p * (1 - (driftPerHour * i));
          const wave = Math.sin(i * 0.7) * (range * 0.25);
          const noise = ((i % 5) - 2) * (range * 0.08);
          const c = Math.max(low * 0.995, Math.min(high * 1.005, center + wave + noise));
          const o = c * (1 + (Math.sin(i) * 0.0008));
          const h = Math.max(o, c) * (1 + Math.abs(Math.cos(i)) * 0.0012);
          const l = Math.min(o, c) * (1 - Math.abs(Math.sin(i * 1.3)) * 0.0012);
          candles.push({ t, o, h, l, c, v: 1000 });
        }
        candleHistory[symbol] = candles;
        saveTradeMemory();
      }

      // Add a fresh micro-candle on tick updates
      const market = livePrices[symbol];
      if (market && market.price) {
        const now = Date.now();
        const lastCandle = candleHistory[symbol][candleHistory[symbol].length - 1];
        if (!lastCandle || (now - lastCandle.t) > 60000) { // update every 1 minute
          candleHistory[symbol].push({
            t: now,
            o: lastCandle ? lastCandle.c : market.price,
            h: Math.max(market.price, market.high || market.price),
            l: Math.min(market.price, market.low || market.price),
            c: market.price,
            v: 100
          });
          if (candleHistory[symbol].length > 100) {
            candleHistory[symbol] = candleHistory[symbol].slice(-100);
          }
          saveTradeMemory();
        }
      }
  ...
  function loadChatHistory() {
    ...
    // Also load trade memory on startup
    loadTradeMemory();
  }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // BIAS-LOCKED DIRECTION ENGINE (Replaces old baseRate comparison)
  // RULE: NO trade against the HTF bias. All 5 engines MUST agree with bias.
  // If bias is NEUTRAL, engines abstain (return null for NO SIGNAL).
  // ═══════════════════════════════════════════════════════════════════════════
  function determineDirection(symbol, engineKey) {
    const marketBias = determineMarketBias(symbol);
    const bias = marketBias.bias;
    const confidence = marketBias.confidence || 50;

    // CRITICAL: If bias is NEUTRAL and confidence is low, abstain
    // This prevents random trades that were destroying win rate
    if (bias === 'NEUTRAL' && confidence < 55) {
      // Use historical winning direction as tiebreaker
      const recentWins = tradeMemory
        .filter(t => t.symbol === symbol && t.result === 'WIN')
        .slice(-10);
      const buyWins = recentWins.filter(t => t.direction === 'BUY').length;
      const sellWins = recentWins.filter(t => t.direction === 'SELL').length;

      if (buyWins > sellWins + 1) return 'BUY';
      if (sellWins > buyWins + 1) return 'SELL';

      // True neutral with no historical edge — favor the 24h change direction
      const market = livePrices[symbol];
      if (market && market.change) {
        const chg = parseFloat(market.change.replace('%', '')) || 0;
        if (chg > 0.2) return 'BUY';
        if (chg < -0.2) return 'SELL';
      }

      // Last resort: use the live price position vs 24h range
      if (market) {
        const p = market.price;
        const h = market.high || p;
        const l = market.low || p;
        const mid = (h + l) / 2;
        return p >= mid ? 'BUY' : 'SELL';
      }

      return 'BUY'; // Absolute fallback
    }

    // ENFORCED: Direction MUST match the bias
    if (bias === 'BULLISH') return 'BUY';
    if (bias === 'BEARISH') return 'SELL';

    // Neutral with decent confidence — use momentum
    const market = livePrices[symbol];
    if (market && market.change) {
      const chg = parseFloat(market.change.replace('%', '')) || 0;
      return chg >= 0 ? 'BUY' : 'SELL';
    }
  ...
  async function fetchLiveMarketData() {
    // 1. Fetch Binance live tickers for Cryptos
    try {
      const binanceSymbols = ['BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'BNBUSDT', 'XRPUSDT', 'DOGEUSDT', 'ADAUSDT', 'AVAXUSDT', 'LINKUSDT', 'SUIUSDT', 'NEARUSDT', 'PEPEUSDT'];
    ...
    // 2. Fetch REAL Live Spot Precious Metals (Gold XAU, Silver XAG, Platinum XPT)
    try {
      ...
      if (goldRes.status === 'fulfilled' && goldRes.value && goldRes.value.price) {
        updateMetalRate('XAU/USD (Gold)', parseFloat(goldRes.value.price));
      }
      if (silverRes.status === 'fulfilled' && silverRes.value && silverRes.value.price) {
        updateMetalRate('XAG/USD (Silver)', parseFloat(silverRes.value.price));
      }
      if (platRes.status === 'fulfilled' && platRes.value && platRes.value.price) {
        updateMetalRate('XPT/USD (Platinum)', parseFloat(platRes.value.price));
      }
    } catch (metalsErr) {}

    // 3. Fetch or update live global Forex & Commodities rates
    try {
    ...
    // 4. Apply institutional micro-tick drift to indices & commodities
    applyLiveTicks();

    // 5. Fetch candle history for bias engine (EMA calculation)
    await fetchCandleHistory(currentSymbol);

    // 6. Build candle history from live ticks for non-Binance assets
    Object.keys(ASSETS).forEach(sym => {
      const asset = ASSETS[sym];
      if (!asset.binance && livePrices[sym] && livePrices[sym].price) {
  ...
  function updateMetalRate(symbol, price) {
    if (!livePrices[symbol] || !price) return;
    const prev = livePrices[symbol].price || price;
    const chg = ((price - prev) / prev) * 100;
    livePrices[symbol].price = price;
    if (Math.abs(chg) > 0.001) {
      livePrices[symbol].change = (chg >= 0 ? '+' : '') + chg.toFixed(2) + '%';
    }
    if (!livePrices[symbol].high || price > livePrices[symbol].high) livePrices[symbol].high = price * 1.006;
    if (!livePrices[symbol].low || price < livePrices[symbol].low) livePrices[symbol].low = price * 0.994;
  }
  ...
  function getModelData(modelKey, symbol, mode) {
    ...
    // 6. UNIFIED MODEL: 5-Engine Consensus Brain (ADAPTIVE Weighted Average)
    // Direction follows bias-locked consensus. Weights adapt based on model performance.
    ...
    // Use adaptive weights (self-improving based on past trade outcomes)
    const w = {
      a: modelWeights.aether9 || 0.20,
      e: modelWeights.evolvex || 0.20,
      s: modelWeights.sentinel || 0.20,
      u: modelWeights.unity || 0.20,
      o: modelWeights.orbit || 0.20
    };
    // Only average entries/TPs/SLs from engines that agree with unified direction
    // This prevents conflicting directions from pulling the trade setup apart
    const agreeingEngines = [];
    if (aetherDir === unifiedDir) agreeingEngines.push({ e: aetherEntry, t: aetherTP, s: aetherSL, w: w.a });
    if (evolveDir === unifiedDir) agreeingEngines.push({ e: evolveEntry, t: evolveTP, s: evolveSL, w: w.e });
    if (sentinelDir === unifiedDir) agreeingEngines.push({ e: sentinelEntry, t: sentinelTP, s: sentinelSL, w: w.s });
    if (unityDir === unifiedDir) agreeingEngines.push({ e: unityEntry, t: unityTP, s: unitySL, w: w.u });
    if (orbitDir === unifiedDir) agreeingEngines.push({ e: orbitEntry, t: orbitTP, s: orbitSL, w: w.o });

    // Normalize weights of agreeing engines
    const totalW = agreeingEngines.reduce((s, eng) => s + eng.w, 0) || 1;
    const unifiedEntry = agreeingEngines.reduce((s, eng) => s + eng.e * (eng.w / totalW), 0) || (aetherEntry * w.a + evolveEntry * w.e + sentinelEntry * w.s + unityEntry * w.u + orbitEntry * w.o);
    const unifiedTP = agreeingEngines.reduce((s, eng) => s + eng.t * (eng.w / totalW), 0) || (aetherTP * w.a + evolveTP * w.e + sentinelTP * w.s + unityTP * w.u + orbitTP * w.o);
    const unifiedSL = agreeingEngines.reduce((s, eng) => s + eng.s * (eng.w / totalW), 0) || (aetherSL * w.a + evolveSL * w.e + sentinelSL * w.s + unitySL * w.u + orbitSL * w.o);
    const unifiedRR = (Math.abs(unifiedTP - unifiedEntry) / Math.abs(unifiedEntry - unifiedSL)).toFixed(2);

    // Get current market bias for metadata
    const currentBias = determineMarketBias(symbol);
    ...
    // Attach bias and performance metadata for self-improving feedback
    selectedModel.biasInfo = currentBias;
    selectedModel.biasLabel = currentBias.bias || 'NEUTRAL';
    selectedModel.biasConfidence = currentBias.confidence || 50;

    // Attach model performance stats (from feedback memory)
    const modelPerf = getModelStats(modelKey === 'unified' ? 'aether9' : modelKey);
    selectedModel.winRate = modelPerf.winRate;
    selectedModel.adaptiveWeight = modelPerf.weight;
    selectedModel.performanceStreak = modelPerf.streak;
    selectedModel.recentResults = modelPerf.lastTrades || [];
  ...
  function renderAnalysisChatMessage(msg) {
    ...
    <div class="chat-message-card rounded-xl border border-border/80 bg-background/95 p-4 space-y-4 shadow-sm transition-all hover:border-border ring-1 ring-foreground/5 animate-in fade-in-50 duration-300" id="${msg.id}" data-msg-id="${msg.id}">
      ...
      <!-- HTF Bias & Model Performance (Self-Improving Feedback) -->
      <div class="rounded-lg border border-border/60 bg-card p-2.5 flex flex-wrap items-center justify-between gap-3 text-xs font-mono">
        <div class="flex items-center gap-3">
          <div class="flex items-center gap-1.5">
            <div class="size-2 rounded-full ${msg.biasLabel === 'BULLISH' ? 'bg-emerald-500' : msg.biasLabel === 'BEARISH' ? 'bg-rose-500' : 'bg-amber-500'}"></div>
            <span class="text-muted-foreground">HTF Bias:</span>
            <strong class="${msg.biasLabel === 'BULLISH' ? 'text-emerald-400' : msg.biasLabel === 'BEARISH' ? 'text-rose-400' : 'text-amber-400'}">${msg.biasLabel || 'NEUTRAL'} (${msg.biasConfidence || 50}%)</strong>
          </div>
          <span class="text-border/60">|</span>
          <div class="flex items-center gap-1.5">
            <span class="text-muted-foreground">Win Rate:</span>
            <strong class="${parseFloat(msg.modelWinRate) >= 60 ? 'text-emerald-400' : parseFloat(msg.modelWinRate) >= 40 ? 'text-amber-400' : 'text-rose-400'}">${msg.modelWinRate || '50.0%'}</strong>
          </div>
          <span class="text-border/60">|</span>
          <div class="flex items-center gap-1.5">
            <span class="text-muted-foreground">Weight:</span>
            <strong class="text-foreground">${msg.adaptiveWeight || '20.0%'}</strong>
          </div>
          ${(msg.recentResults && msg.recentResults.length > 0) ? `
          <span class="text-border/60">|</span>
          <div class="flex items-center gap-0.5">
            ${msg.recentResults.map(r => `<span class="size-4 rounded-sm text-[9px] font-bold flex items-center justify-center ${r === 'W' ? 'bg-emerald-500/20 text-emerald-400' : r === 'L' ? 'bg-rose-500/20 text-rose-400' : 'bg-amber-500/20 text-amber-400'}">${r}</span>`).join('')}
          </div>` : ''}
        </div>
      </div>
  ...
  function createAndAppendAnalysis(symbol, modelKey, mode) {
    ...
    const msg = {
      ...
      biasLabel: model.biasLabel || 'NEUTRAL',
      biasConfidence: model.biasConfidence || 50,
      modelWinRate: (model.winRate || 50).toFixed(1) + '%',
      adaptiveWeight: ((model.adaptiveWeight || 0.20) * 100).toFixed(1) + '%',
      performanceStreak: model.performanceStreak || 0,
      recentResults: model.recentResults || []
    };
  ...
  function handleChatFeedback(msgId, outcome) {
    ...
    // Feed manual outcome into trade memory for self-improvement
    const result = (outcome === 'Hit Target' || outcome === 'Partial Profit') ? 'WIN' :
                   (outcome === 'Stopped Out' || outcome === 'Closed Early (Loss)') ? 'LOSS' : 'BE';

    // Only record if not already in memory (prevent duplicates from auto-tracking)
    const alreadyRecorded = tradeMemory.some(t =>
      t.symbol === msg.symbol && t.modelKey === msg.modelKey &&
      Math.abs(t.timestamp - new Date(msg.date + ' ' + msg.timestamp).getTime()) < 60000
    );
    ...
    if (!alreadyRecorded) {
      recordTradeOutcome({
        ...
        entry: parseFloat(String(msg.entry).replace(/[^0-9.-]/g, '')) || 0,
        tp: parseFloat(String(msg.tp3 || msg.tp2).replace(/[^0-9.-]/g, '')) || 0,
        sl: parseFloat(String(msg.sl).replace(/[^0-9.-]/g, '')) || 0,
        result: result,
        exitPrice: 0,
        pnlPct: result === 'WIN' ? 2.5 : result === 'LOSS' ? -1.0 : 0,
        bias: msg.biasLabel || 'NEUTRAL',
        biasConfidence: msg.biasConfidence || 50,
        timestamp: Date.now(),
        duration: 0,
        logic: msg.logic,
  ...
  // Multi-Trade Lifecycle & Real-Time Active Trade Tracker (Self-Improving)
  let tradeCounter = 1;
  ...
  function initActiveTrade() {
    ...
    const p = market.price;
    const bias = determineMarketBias(currentSymbol);
    ...
    const dir = model.direction || model.signalType || 'BUY';
  ...
  // Auto-record trade outcome to feedback memory (called once per terminal state)
  function autoRecordTradeOutcome(result, exitPrice) {
    if (!activeTrade || activeTrade.outcomeRecorded) return;
    activeTrade.outcomeRecorded = true;

    const duration = Date.now() - activeTrade.timestamp;
    const pnlPct = activeTrade.direction === 'SELL'
      ? ((activeTrade.entry - exitPrice) / activeTrade.entry) * 100
      : ((exitPrice - activeTrade.entry) / activeTrade.entry) * 100;

    const tradeRecord = {
      id: activeTrade.id,
      symbol: activeTrade.symbol,
      modelKey: activeTrade.modelKey,
      ...
      exitPrice: exitPrice,
      pnlPct: pnlPct,
      maxFavorablePnl: activeTrade.maxFavorablePnl,
      maxAdversePnl: activeTrade.maxAdversePnl,
      bias: activeTrade.bias,
      biasConfidence: activeTrade.biasConfidence,
      timestamp: activeTrade.timestamp,
      duration: duration,
      logic: activeTrade.logic
    };

    // Classify mistakes for losing trades
    if (result === 'LOSS') {
      tradeRecord.mistakes = classifyMistake(tradeRecord);
      console.log(`[FEEDBACK] Trade LOSS on ${activeTrade.symbol} via ${activeTrade.modelKey}. Mistakes: ${tradeRecord.mistakes.join(', ')}`);
    } else {
      console.log(`[FEEDBACK] Trade WIN on ${activeTrade.symbol} via ${activeTrade.modelKey}. PnL: +${pnlPct.toFixed(2)}%`);
    }

    // Feed outcome back to memory system — this triggers weight recalculation
    recordTradeOutcome(tradeRecord);
  }
  ...
  function evaluateActiveTrade(currentPrice) {
    ...
    const dir = activeTrade.direction || 'BUY';

    // Direction-aware PnL calculation
    const pnlPct = dir === 'SELL'
      ? ((entry - currentPrice) / entry) * 100
      : ((currentPrice - entry) / entry) * 100;
    activeTrade.pnlPct = pnlPct;

    // Track max favorable and max adverse excursion
    if (pnlPct > activeTrade.maxFavorablePnl) activeTrade.maxFavorablePnl = pnlPct;
    if (pnlPct < activeTrade.maxAdversePnl) activeTrade.maxAdversePnl = pnlPct;
    ...
    const progressTp1 = dir === 'SELL'
      ? Math.min(100, Math.max(0, ((entry - currentPrice) / (entry - tp1)) * 100))
      : Math.min(100, Math.max(0, ((currentPrice - entry) / (tp1 - entry)) * 100));
    ...
    // Direction-aware TP/SL hit detection
    const tp2Hit = dir === 'SELL' ? currentPrice <= tp2 : currentPrice >= tp2;
    const tp1Hit = dir === 'SELL' ? currentPrice <= tp1 : currentPrice >= tp1;
    const slHit = dir === 'SELL' ? currentPrice >= sl : currentPrice <= sl;

    if (tp2Hit) {
      activeTrade.status = 'TP2_HIT';
      autoRecordTradeOutcome('WIN', currentPrice);
      if (statusBadge) {
      ...
      if (alertEl) {
        alertEl.innerHTML = `<strong>FULL TARGET ACHIEVED:</strong> Price reached TP 2 (${formatAssetPrice(tp2, currentSymbol)}). Total gain <strong>+${pnlPct.toFixed(2)}%</strong>. Outcome auto-recorded to feedback memory. Model weight for ${activeTrade.modelKey} updated. Click <strong>[Analyze Next Trade]</strong> to scan the next market opportunity.`;
      }
      if (tp2StatusEl) {
      ...
      autoRecordTradeOutcome('LOSS', currentPrice);
      if (statusBadge) {
      ...
      if (alertEl) {
        const mistakes = classifyMistake(activeTrade);
        const mistakeText = mistakes.includes('COUNTER_TREND') ? ' <strong style="color:#f59e0b">[COUNTER-TREND DETECTED]</strong> Future signals will be bias-locked.' :
          mistakes.includes('BAD_ENTRY') ? ' <strong style="color:#f59e0b">[BAD ENTRY]</strong> Model entry offset will be refined.' :
          mistakes.includes('SL_TOO_TIGHT') ? ' <strong style="color:#f59e0b">[SL TOO TIGHT]</strong> Consider wider invalidation.' : '';
        alertEl.innerHTML = `<strong>STOP LOSS TRIGGERED:</strong> Invalidation floor (${formatAssetPrice(sl, currentSymbol)}) touched. Position closed defensively. Loss recorded to feedback memory.${mistakeText} Click <strong>[Analyze Next Trade]</strong> to find the next setup.`;
      }
      if (pulseEl) pulseEl.className = 'size-2 rounded-full bg-rose-500';
    } else {
      // Trade still running — show direction-aware guidance
      if (statusBadge) {
      ...
      if (alertEl) {
        // Add trade health warning if price is 70%+ towards SL
        const slProgress = dir === 'SELL'
          ? ((currentPrice - entry) / (sl - entry)) * 100
          : ((entry - currentPrice) / (entry - sl)) * 100;
        const healthWarning = slProgress > 70 ? ` <span style="color:#ef4444;font-weight:bold">[HIGH RISK: ${slProgress.toFixed(0)}% to SL]</span>` : '';
        ...
        if (pnlPct >= 0) {
          ...
          alertEl.innerHTML = `<strong>Guidance:</strong> Position active near entry (${formatAssetPrice(entry, currentSymbol)}). Stop loss is protected at ${formatAssetPrice(sl, currentSymbol)} (-${Math.abs(pnlPct).toFixed(2)}%).${healthWarning} Maintain discipline and avoid premature manual intervention.`;
        }
      }
      if (tp1StatusEl) {
